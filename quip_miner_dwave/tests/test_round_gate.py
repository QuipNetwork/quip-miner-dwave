...
def _verdict(join: bool, reason: str) -> RoundDecision:
    return RoundDecision(
        join=join, reason=reason, slot=13, p_now=0.024, expected_jobs=165.0,
        jobs_per_s=1.1, lam=1.5e-4, p_best=0.046, wait_slot=14, wait_rounds=6,
        saturates_in_rounds=10,
    )
...
def test_the_strategy_is_consulted_only_when_the_budget_allows():
    strategy = FakeStrategy([_verdict(True, REASON_GOOD_SHOT)])
    ...
    gate2, _ = _gate(FakeStrategy([_verdict(True, REASON_GOOD_SHOT)]))
    result = gate2.on_qblock_boundary(100, MID_PERIOD)
    assert result is not None and result.allowed and result.changed
    assert result.round is not None and result.round.reason == REASON_GOOD_SHOT


def test_a_skip_verdict_withholds_credits():
    strategy = FakeStrategy([_verdict(False, REASON_BETTER_SLOT)])
    gate, _ = _gate(strategy)
    result = gate.on_qblock_boundary(100, MID_PERIOD)
    assert result is not None
    assert not result.allowed and not result.changed and not gate.participating
    assert result.round is not None and result.round.reason == REASON_BETTER_SLOT
    assert strategy.calls[0].headroom_us > 0


def test_a_skip_after_participating_parks_credits():
    gate, _ = _gate(FakeStrategy([_verdict(True, REASON_GOOD_SHOT), _verdict(False, REASON_BETTER_SLOT)]))
    ...
    # The mid-round stop is untouched: with participation off, a job is refused.
...
def test_a_skipped_round_is_logged_and_recorded(monkeypatch, tmp_path, caplog):
    monkeypatch.setattr(
        RoundStrategy, "decide", lambda self, now, budget: _verdict(False, REASON_BETTER_SLOT)
    )
    ...
    assert any("skip (better-slot)" in r.getMessage() for r in caplog.records)
    rows = {r["generation"]: r for r in HistoryStore(usage_db).rounds(since_ts=0, limit=10)}
    assert rows[2]["joined"] == 0 and rows[2]["reason"] == REASON_BETTER_SLOT
    assert rows[2]["p_win"] == 0.024 and rows[2]["expected_jobs"] == 165.0
